...
- **`KC_WITH_TIMESTAMP` + `KC_NOCHECKCERTTIME` не работает** (замерено 2026-08-23 на
  2.0.13, тестовый ключ, тестовый TSA): выход 2–6 байт при `rc==0`. Та же подпись с
  включённой проверкой времени серта — 4471–4483 байта. Комбинация не даёт ошибки,
  только пустоту, поэтому единственная защита — проверка длины.
  > **Реализовано:** драйвер подписывает CMS **всегда с `KC_OUT_PEM`** и снимает
  > обёртку сам, когда просят DER; буфер проверяется **по виду, а не по длине** —
  > PEM-обрамление плюс ASN.1 `SEQUENCE` внутри, иначе `provider.ErrEmptyOutput`
  > (каталог: `output.empty`). Порог по длине не годится: при неподдерживаемой
  > комбинации библиотека оставляет буфер как есть, и наружу идёт то, что лежало на
  > стеке — иногда ноль байт, иногда шесть (`90 64 2c ff ff 7f`), и длина не наша,
  > чтобы её предсказывать.

## Загрузка доверенных CA чувствительна к ПОРЯДКУ

**Замерено 2026-08-23 на 2.0.13.** `SignData` разрешает цепочку по сертификатам,
загруженным в стор инстанса, **в порядке загрузки**, и сдаётся, если впереди нужного
оказался посторонний CA:

| набор якорей | подпись |
|---|---|
| корень+промежуточный подписанта, пустой стор | ✅ 2244 байта |
| они же **плюс** 29 якорей реестра РК **после** них | ✅ |
| **один посторонний корень перед** ними | ❌ `0x08F00042` |
| 3…32 якоря реестра впереди | ❌ на любом количестве |

Ошибка приходит как `X509 Load certificate from file - OK. ERROR 0x8f00042: Load
certificate from system store - failed to load root or intermediate certificate` —
то есть выглядит как проблема сертификата подписанта, хотя дело в соседях по стору.
Ни один якорь реестра **по отдельности** подпись не ломает — ломает позиция.

> **Следствие:** включённый `trust.use-rk-registry` делал подписание невозможным
> целиком. **Реализовано:** при отказе с anchor-ошибкой домен пересобирает попытку,
> подавая **только цепочку самого подписанта** (`core.signerChainAnchors` — экспорт
> собственного серта ключа, построение цепочки по стору и AIA). Большой trust-store
> для подписи не просто избыточен — он и есть поломка.
