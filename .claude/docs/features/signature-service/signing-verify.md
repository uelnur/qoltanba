...
- **Пустой выход при `rc==0` — отказ, а не подпись** (`provider.ErrEmptyOutput`,
  каталог `output.empty`). Иначе вызывающий получал пустой контейнер с проставленным
  `cadesLevel: T` и узнавал об этом только когда verify отвечал «недействительна» —
  выглядело как порочная подпись, а не как отказ подписания. Известный триггер —
  `withTimestamp` вместе с `noCheckCertTime`.
