...
## Кодировки контейнера (PEM/DER) и молчаливые пустые выходы

Найдено первым интегратором, воспроизведено против реальной библиотеки. Все три
случая были **молчаливыми**: `rc==0`, `libError: null`, HTTP 200.

- **Подпись CMS всегда идёт с `KC_OUT_PEM`.** Библиотека наполняет выходной буфер
  только под этим флагом; без него (и даже с `KC_OUT_DER`) — пустой/усечённый blob
  при `rc==0`. Запрошенный `outputPem: false` обслуживается снятием PEM-обёртки в
  драйвере (`internal/native.pemToDER`), а не флагом. Каветы самой библиотеки —
  [`../../kalkan-api/signing.md`](../../kalkan-api/signing.md).
- **Выход проверяется по виду, а не по длине** (`provider.ErrEmptyOutput`, каталог
  `output.empty`): PEM-обрамление плюс ASN.1 `SEQUENCE` внутри. При неподдерживаемой
  комбинации библиотека оставляет буфер нетронутым и отдаёт то, что лежало на стеке —
  иногда ноль байт, иногда шесть; порог по длине пропускал второй случай, а длина
  мусора не наша, чтобы её предсказывать. Известный триггер — `withTimestamp` вместе
  с `noCheckCertTime`.
- **Подпись пересобирается на цепочке подписанта, если стор помешал.** Библиотека
  разрешает цепочку по порядку загрузки якорей и сдаётся, когда впереди нужного
  оказался посторонний CA — один чужой корень впереди ломает подпись, а те же
  сертификаты с цепочкой подписанта первыми проходят при любом размере стора. Отсюда
  `trust.use-rk-registry` делал подписание невозможным целиком. При отказе с
  anchor-ошибкой домен повторяет попытку, подавая **только цепочку самого подписанта**
  (`signerChainAnchors`: экспорт серта ключа → цепочка по стору и AIA). Замеры —
  [`../../kalkan-api/signing.md`](../../kalkan-api/signing.md).
- **Кодировка входа называется явно** (`kcInDER`, а не «ничего не передаём»). Раньше
  для DER-контейнера `VerifyData` отрабатывал, а обход подписантов
  (`KC_GetCertFromCMS`) отказывал — verify возвращал `valid: true` с **пустым**
  `signers`. Сверх флага домен держит гейт: **вердикт, у которого нет ни одного
  подписанта, не может быть `valid`** — он не называет никого, а это хуже отказа для
  того, кто читает только `valid`.
- **Восстановленное содержимое больше не режется по NUL.** Буфер verify прогонялся
  через обрезку по первому нулевому байту (уместную для PEM-текста), из-за чего
  `extractContent` над бинарником — PDF, ZIP — молча терял всё после первого нуля.
...
- Остальные поля TSP (serialNumber, policy, tsa, hashAlg, imprint) — **разбором
  ...
  >
  > 3. **Политика TSA — опционально, решением оператора.** `trust.tsa-policies`
  >    (список OID, пусто по умолчанию) ограничивает, под какой политикой выпущенная
  >    метка засчитывается как `T`. Пусто — **ничего не навязываем и это честно**: все
  >    политики НУЦ цепляются к одним якорям, так что выбор между ними — это вопрос
  >    приемлемых алгоритмов (напр. «только ГОСТ-2015, не RSA»), а не факт, который
  >    сервис может вывести. Отдаётся как `timestamp.policyName` (имя из реестра) +
  >    `policyAccepted` (tri-state: `nil` = список не настроен) + `policyNote`.
  >    Конфиг валидируется: OID вне арки `1.2.398.3.3.2.6` отвергается на старте.
  >
  > `T` — только когда imprint и подпись TSA оба `true` **и** политика не отвергнута
  > (при настроенном списке); иначе `BES` + warning с причиной. Fallback по
