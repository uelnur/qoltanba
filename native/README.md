...
> e2e-тестов — воплощение принципа Bring-Your-Own-Library (см.
> `.claude/docs/kalkan-api/` и `qoltanba/docs/DESIGN.md` §8/§13).

Извлечено из официального SDK НУЦ РК (`SDK 2.0`, C-API). Сам SDK-каталог удалён —
здесь оставлено только нужное; полный архив SDK — `../SDK (2).7z`.
...
`../SDK (2).7z` (`C/Linux/C/libs/v2.0.2 (Сертифицированная версия)/`).
